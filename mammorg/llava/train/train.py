...
from llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, SPECIAL_PHRASES
...
class ModelArguments:
    ...
    pretrain: Optional[str] = field(default=None)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    aux_only: bool = field(default=False)
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                mm_projector1_folder = os.path.join(parent_folder, "mm_projector1")
                os.makedirs(mm_projector1_folder, exist_ok=True)
                torch.save(weight_to_save, os.path.join(mm_projector1_folder, f'{current_folder}.bin'))
                mm_projector2_folder = os.path.join(parent_folder, "mm_projector2")
                os.makedirs(mm_projector2_folder, exist_ok=True)
                torch.save(weight_to_save, os.path.join(mm_projector2_folder, f'{current_folder}.bin'))
            else:
                torch.save(weight_to_save, os.path.join(output_dir, f'mm_projector1.bin'))
                torch.save(weight_to_save, os.path.join(output_dir, f'mm_projector2.bin'))
...
def safe_save_probing(trainer: transformers.Trainer,
                    output_dir: str):
    """Collects the state dict and dump to disk."""

    probing_weights = {
        name: param.data for name, param in trainer.model.named_parameters() 
        if 'probing' in name
    }
    os.makedirs(output_dir, exist_ok=True)
    torch.save(probing_weights, os.path.join(output_dir, "aux_probing.bin"))

def safe_save_cross_module(trainer: transformers.Trainer,
                    output_dir: str):
    """Collects the state dict and dump to disk."""

    probing_weights = {
        name: param.data for name, param in trainer.model.named_parameters() 
        if 'cross_module' in name
    }
    os.makedirs(output_dir, exist_ok=True)
    torch.save(probing_weights, os.path.join(output_dir, "cross_module.bin"))
def safe_save_graph_model(trainer: transformers.Trainer,
                    output_dir: str):
    """Collects the state dict and dump to disk."""

    probing_weights = {
        name: param.data for name, param in trainer.model.named_parameters() 
        if 'graph_model' in name
    }
    os.makedirs(output_dir, exist_ok=True)
    torch.save(probing_weights, os.path.join(output_dir, "graph_model.bin"))
...
def smart_tokenizer_and_embedding_resize_normal(
    tokens_list: List,
    tokenizer: transformers.PreTrainedTokenizer,
    model: transformers.PreTrainedModel
):
    ...
    Note: This is the unoptimized version that may make your embedding size not be divisible by 64.
    """
    num_new_tokens = tokenizer.add_tokens(tokens_list)
    model.resize_token_embeddings(len(tokenizer))
    ...
    if num_new_tokens > 0:
        ...
        input_embeddings[-num_new_tokens:] = input_embeddings_avg + 0.01 * torch.randn_like(input_embeddings[-num_new_tokens:])
        output_embeddings[-num_new_tokens:] = output_embeddings_avg + 0.01 * torch.randn_like(output_embeddings[-num_new_tokens:])
    ...
    for source in sources:
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value']:
                ...
                sentence['value'] = DEFAULT_IMAGE_TOKEN*num_images + '\n' + sentence['value']
...
def preprocess_plain(
    ...
    for source in sources:
        ...
        num_images = source[0]['value'].count(DEFAULT_IMAGE_TOKEN)
        source[0]['value'] = DEFAULT_IMAGE_TOKEN*num_images
...
class LazySupervisedDataset(Dataset):
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            img_tokens = 256*4 if 'Image_paths' in sample else 0
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            cur_len = cur_len if 'Image_paths' in sample else -cur_len
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'Image_paths' in sources[0]:
            ...
            for view in ['R_CC', 'R_MLO', 'L_CC', 'L_MLO']:  # 确保固定顺序处理4个视图
                if view in image_file:
                    ...
                    if image is None:
                        logging.error(f"Failed to load {view} image, using empty image.")
                        image = Image.new('RGB', (512, 512), tuple(int(x*255) for x in processor.image_mean))
        ...
        if 'Image_paths' in self.list_data_dict[i]:
            data_dict['image'] = images
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
            for view in data_dict['image']:
                data_dict['image'][view] = torch.zeros(3, crop_size['height'], crop_size['width'])
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        aux_labels = {
            ...
            'right_birads_logits': torch.cat([inst['aux_labels']['right_birads_logits'] for inst in instances]),
            'right_status_logits': torch.stack([
                inst['aux_labels']['right_status_logits'].view(-1)
                for inst in instances
            ]),
            'located_at_logits': torch.stack([
                inst['aux_labels']['located_at_logits'] 
                for inst in instances
            ]),
        ...
        if 'image' in instances[0]:
            images_list = [instance['image'] for instance in instances]
            if all(
                isinstance(x, dict) and 
                set(x.keys()) == {'R_CC', 'R_MLO', 'L_CC', 'L_MLO'} and
                all(
                    x[view].shape == images_list[0][view].shape 
                ...
                batch_images = {
                    ...
                    'L_CC': torch.stack([x['L_CC'] for x in images_list]),
                    'L_MLO': torch.stack([x['L_MLO'] for x in images_list])
                }
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        print('ready for initialize_vision_tokenizer')
            ...
            if 'lm_head' in name or 'embed_tokens' in name:
                if hasattr(module, 'weight'):
                    if training_args.bf16 and module.weight.dtype == torch.float32:
                        module = module.to(torch.bfloat16)
    for t in SPECIAL_PHRASES:
        print("special token:", t, "-> id:", tokenizer.convert_tokens_to_ids(t))
